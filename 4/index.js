...
const getWinnerRow = (board) =>
  board.find((row) => row.every((number) => number !== "."));
const getWinnerColumn = (board) => getWinnerRow(transpose2DArray(board));

const isWinnerBoard = (board) =>
  Boolean(getWinnerRow(board) || getWinnerColumn(board));

const solve1 = (numbers, boards) => {
  const marked = Array.from(boards, (board) =>
    Array.from(board, (row) => Array.from(row, () => "."))
  );
  let win = false;
  let winner_number = -1;
  ...
  while (!win) {
    ...
    boards.forEach((board, i) => {
      board.forEach((row, j) => {
        marked[i][j] = row.map((number, index) =>
          number === current_number ? number : marked[i][j][index]
        );
      });
