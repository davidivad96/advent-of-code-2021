...
const solve1 = (patterns, outputs) => {
  return outputs.reduce((prev, curr) => {
    const unique_digits = curr.filter((digit) =>
...
const calculateNumbers = (pattern) => {
  ...
  const nine = pattern_copy.find(
    (segments) =>
      segments.length === 6 && numberOfCharactersIncluded(segments, four) === 4
  );
  pattern_copy.splice(pattern_copy.indexOf(nine), 1);
  const zero = pattern_copy.find(
    (segments) =>
      segments.length === 6 && numberOfCharactersIncluded(segments, one) === 2
  );
  ...
  const three = pattern_copy.find(
    (segments) =>
      segments.length === 5 && numberOfCharactersIncluded(segments, one) === 2
  );
  pattern_copy.splice(pattern_copy.indexOf(three), 1);
  const five = pattern_copy.find(
    (segments) =>
      segments.length === 5 && numberOfCharactersIncluded(segments, four) === 3
  );
...
const decodeOutput = (numbers, output) => {
  ...
  for (const digit of output) {
    number = number.concat(
      numbers.findIndex(
        (n) =>
          n.length === digit.length &&
          numberOfCharactersIncluded(n, digit) === digit.length
      )
    );
  }
