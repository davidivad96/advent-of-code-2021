...
const mostCommonBitAtIndex = (input, i) => {
  const count_one = input.reduce(
    (prev, curr) => (curr.charAt(i) === "1" ? prev + 1 : prev),
    0
  );
...
const solve2 = (input) => {
  ...
  const calculateRatings = (oxygen_report, co2_report, index) => {
    ...
    if (oxygen_report.length > 1) {
      ...
      new_oxygen_report = oxygen_report.filter(
        (value) => value.charAt(index) === oxygen_most_common
      );
    }
    if (co2_report.length > 1) {
      ...
      new_co2_report = co2_report.filter(
        (value) => value.charAt(index) === co2_least_common
      );
    }
    index++;
    return calculateRatings(new_oxygen_report, new_co2_report, index);
  };
  const [oxygen_generator_rating, co2_scrubber_rating] = calculateRatings(
    input,
    input,
    i
  );
